...
/**
 * @brief Set aw9523 port interrupt
 * @param port
 * @param config_byte D[7:0] set Px_7 ~ Px_0, 0: interrupt enable; 1: interrupt disable
 */
void aw9523_set_port_interrupt(AW9523_Port_t port, uint8_t config_byte)
{
    esp_err_t ret = aw9523_reg_write_byte((port == AW9523_PORT_0) ? AW9523_REG_INT_P0 : AW9523_REG_INT_P1, config_byte);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Set port interrupt failed");
        return;
    }
}

/**
 * @brief Set aw9523 port_pin interrupt
 * @param port
 * @param pin_num
 * @param config_bit 0: interrupt enable; 1: interrupt disable
 */
void aw9523_set_pin_interrupt(AW9523_Port_t port, AW9523_Pin_t pin_num, bool config_bit)
{
    uint8_t reg = (port == AW9523_PORT_0) ? AW9523_REG_INT_P0 : AW9523_REG_INT_P1;
    uint8_t port_interrupt;
    esp_err_t ret = aw9523_reg_read(reg, &port_interrupt, sizeof(port_interrupt));
    if (config_bit == 1)
    {
        port_interrupt |= (1 << pin_num);
    }
    else
    {
        port_interrupt &= ~(1 << pin_num);
    }
    ret = aw9523_reg_write_byte(reg, port_interrupt);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Set pin interrupt failed");
        return;
    }
}
...
void aw9523_Demo(void)
{
    ...
    aw9523_set_port_inout(AW9523_PORT_1, 0x00);
